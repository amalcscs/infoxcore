...
def hrtasks1(request):
    ...
    if request.session.has_key('usernamehr2'):
        ...
        usernamehr2 = "dummy"
    names = HRtasks.objects.filter(branch=usernamehr2)
    mem = regdetails.objects.filter(designation=usernamehr).filter(name=usernamehr1)
...
def hrmytasks(request):
    ...
    if HRtasks.objects.filter(id=task_id):
...
# def hrmytasks(request):
#      return render(request, 'hrmy tasks.html')
...
def hrgiveprojects(request):
    ...
    if request.session.has_key('usernamehr2'):
        ...
        usernamehr2 = "dummy"
    department1=mangivetasks.objects.values('department').filter(~Q(department='HR')).filter(~Q(department='')).filter(branch=usernamehr2).distinct()
    mem=regdetails.objects.filter(designation=usernamehr) .filter(name=usernamehr1)
...
def hrassignedtasks(request):
    ...
    #names = mangivetasks.objects.filter(assignedBy='Executive')
    mem=regdetails.objects.filter(designation=usernamehr) .filter(name=usernamehr1)
    context={'names':names,'mem':mem}
